...
class NetworkDevie(models.Model):
    '''网络设备'''
    sub_asset_type_choice = (
        (0, '路由器'),
        (1, '交换机'),
    ...
    asset = models.OneToOneField('Asset')
    sub_asset_type = models.IntegerField(choices=sub_asset_type_choice, default=0, verbose_name="网络设备类型")
...
class IDC(models.Model):
    ...
    memo = models.TextField(max_length=128, blank=True, null=True, verbose_name="备注")
...
class Contract(models.Model):
    ...
    datail = models.TextField("合同详细", blank=True, null=True)
...
class RAM(models.Model):
    ...
    model = models.CharField("内存型号", max_length=1128, blank=True, null=True)
    manufacture = models.CharField("内存制造商", max_length=128, blank=True, null=True)
...
class EventLog(models.Model):
    ...
    new_asset = models.ForeignKey('NewAssetApprovalZone', blank=True, null=True, on_delete=models.SET)
    event_type = models.SmallIntegerField("事件类型", choices=event_type_choice, default=4)
    componet = models.CharField("事件子项", max_length=256, blank=True, null=True)
    detail = models.TextField("事件详情")
    date = models.DateTimeField("事件时间", auto_now_add=True)
    user = models.ForeignKey(User, blank=True, null=True, verbose_name="时间执行人", on_delete=models.SET_NULL)
    memo = models.TextField("备注", blank=True, null=True)
